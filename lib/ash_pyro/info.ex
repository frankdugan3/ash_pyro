defmodule AshPyro.Info do
  @moduledoc """
  Helpers to introspect `AshPyro` modules. Intended for use in components that automatically build UI from resource configuration.
  """
  ...
  alias AshPyro.Dsl.DataTable
  alias AshPyro.Form.Action
  alias Spark.Dsl.Extension

  @doc """
  Returns the form fields defined in the `AshPyro.Dsl` extension for the given action.

  ## Examples

      iex> form_for(AshPyro.InfoTest.UserPage, :create) |> Map.get(:fields) |> Enum.map(& &1.label)
      ["Primary Info", "Authorization", "Friendships", "Notes"]
  """
  @spec form_for(AshPyro.t(), atom()) :: Action.t() | nil
  def form_for(ash_pyro, action_name) do
    ash_pyro
  ...
  @doc """
  Returns the data table defined in the `AshPyro.Dsl` extension for the given action.

  ## Examples

      iex> data_table_for(AshPyro.InfoTest.UserPage, :list) |> Map.get(:name)
      :list
  """
  @spec data_table_for(AshPyro.t(), atom()) ::
          [
            DataTable
          ]
          | nil
